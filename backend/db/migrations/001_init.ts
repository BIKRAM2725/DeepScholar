...
import { papersTable } from "../schema/papers.schema.ts";

export const runMigrations = async () => {
  await db.query(usersTable);
  await db.query(chatsTable);
  await db.query(messagesTable);
  await db.query(papersTable);
  ...
  await db.query(`CREATE INDEX IF NOT EXISTS idx_papers_user_id ON papers(user_id)`);
  await db.query(`CREATE INDEX IF NOT EXISTS idx_papers_user_time ON papers(user_id, created_at DESC)`);
};